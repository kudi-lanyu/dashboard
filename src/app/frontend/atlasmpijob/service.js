...
/** @final */
export class AtlasMpiJobService {
  /**
  ...
  deploy(spec) {
    ...
    let tokenPromise = this.tokenService_.getTokenForAction('atlas');

    tokenPromise.then(
        (token) => {
          /** @type {!angular.Resource} */
          let resource = this.resource_(
              'api/v1/atlas/task/mpijob', {},
        ...
        (err) => {
          defer.reject(err);
          this.log_.error('Get Token failed. Error deploying application:', err);
        });
