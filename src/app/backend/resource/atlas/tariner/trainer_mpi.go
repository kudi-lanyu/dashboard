...
func (mj *TrainerMPIJob) Name() string {
	return mj.name
}

// Get the chief Pod of the Job.
func (mj *TrainerMPIJob) ChiefPod() v1.Pod {
	return mj.chiefPod
}
...
func (mj *TrainerMPIJob) Trainer() string {
	return mj.trainerType
}

// Get all the pods of the Training Job
func (mj *TrainerMPIJob) AllPods() []v1.Pod {
	return mj.pods
}

// Get the Status of the Job: RUNNING, PENDING, SUCCEEDED, FAILED
func (mj *TrainerMPIJob) GetStatus() (status string) {
	status = "UNKNOWN"
	if mj.mpijob.Name == "" {
		return status
	}
...
// Get the start time
func (mj *TrainerMPIJob) StartTime() *metav1.Time {
	return &mj.mpijob.CreationTimestamp
}

// Get the Job Age
func (mj *TrainerMPIJob) Age() string {
	job := mj.mpijob
...
func (mj *TrainerMPIJob) RequestedGPU() int64 {
	if mj.requestedGPU > 0 {
		return mj.requestedGPU
	}
	for _, pod := range mj.pods {
		mj.requestedGPU += atlasutil.GpuInPod(pod)
	}
	return mj.requestedGPU
}

// Requested GPU count of the Job
func (mj TrainerMPIJob) AllocatedGPU() int64 {
	if mj.allocatedGPU > 0 {
		return mj.allocatedGPU
	}
	for _, pod := range mj.pods {
		mj.allocatedGPU += atlasutil.GpuInActivePod(pod)
	}
	return mj.allocatedGPU
}

// Get the hostIP of the chief Pod
func (mj TrainerMPIJob) HostIPOfChief() (hostIP string) {
	hostIP = "N/A"
