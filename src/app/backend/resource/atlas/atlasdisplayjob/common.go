...
type MPIJobCell v1alpha1.MPIJob

// GetProperty allows to perform complex data section on PersistentVolumeCell.
func (self MPIJobCell) GetProperty(name dataselect.PropertyName) dataselect.ComparableValue {
	switch name {
...
func toCells(std []v1alpha1.MPIJob) []dataselect.DataCell {
	cells := make([]dataselect.DataCell, len(std))
	for i := range std {
		cells[i] = MPIJobCell(std[i])
	}
	return cells
}

// fromCells converts cells []dataselect.DataCell to []api.PersistentVolume.
func fromCells(cells []dataselect.DataCell) []v1alpha1.MPIJob {
	std := make([]v1alpha1.MPIJob, len(cells))
	for i := range std {
		std[i] = v1alpha1.MPIJob(cells[i].(MPIJobCell))
	}
	return std
}
