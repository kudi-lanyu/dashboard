...
import (
	...
	atlasmpijob "github.com/kubernetes/dashboard/src/app/backend/resource/atlas/atlasjob/mpijob"
	atlasinteractive "github.com/kubernetes/dashboard/src/app/backend/resource/atlas/interactive"
)
...
func CreateHTTPAPIHandler(iManager integration.IntegrationManager, cManager clientapi.ClientManager,
	...
	// atlas display
	apiV1Ws.Route(
		apiV1Ws.GET("/atlas/display").To(apiHandler.handleAtlasDisplay).Writes(cluster.Cluster{}))
	apiV1Ws.Route(
		apiV1Ws.GET("/atlas/fake").To(apiHandler.handleAtlasDisplayFakeList).Writes(atlasinteractive.ExampleList{}))
	apiV1Ws.Route(
		apiV1Ws.GET("/atlas/job/{namespace}").To(apiHandler.handleAtlasDisplayJobList).Writes(atlasjob.AtlasMpiJobList{}))

	apiV1Ws.Route(
		apiV1Ws.GET("/atlas/job/{namespace}/{name}").To(apiHandler.handleAtlasDisplayJobInfo).Writes(atlasjob.AtlasMpiJob{}))
	...
	apiV1Ws.Route(
		apiV1Ws.GET("/atlas/data/{namespace}").To(apiHandler.handleAtlasDisplayDataList).Writes(atlasdata.AtlasDataList{}))

	// atlas task
	apiV1Ws.Route(apiV1Ws.GET("/atlas/tasks").To(apiHandler.handleAtlasTasks).Writes(cluster.Cluster{}))

	apiV1Ws.Route(apiV1Ws.POST("/atlas/task/mpijob").To(apiHandler.handleAtlasTasksMpiJob).Reads(atlasmpijob.MPIJobArgs{}).
		Writes(atlasmpijob.MPIJobArgs{}))

	// atlas fake example
	apiV1Ws.Route(
		apiV1Ws.POST("/atlas/example").
			To(apiHandler.handleDeployAtlasExample).
			Reads(atlasinteractive.DeploySpec{}).
			Writes(atlasinteractive.DeploySpec{}))
...
func (apiHandler *APIHandler) handleGetCluster(request *restful.Request, response *restful.Response) {
	...
	log.Println("cluster debuginfo:===============>\n" + debugInfo(result))
	response.WriteHeaderAndEntity(http.StatusOK, result)
}

func (apiHandler *APIHandler) handleAtlasDisplay(request *restful.Request, response *restful.Response) {
	k8sClient, err := apiHandler.cManager.Client(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	...
	log.Println("atlas display debuginfo:===============>\n" + debugInfo(result))
	response.WriteHeaderAndEntity(http.StatusOK, result)
}
func (apiHandler *APIHandler) handleAtlasTasks(request *restful.Request, response *restful.Response) {
	k8sClient, err := apiHandler.cManager.Client(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	...
	log.Println("atlas tasks debuginfo:===============>\n" + debugInfo(result))
...
func (apiHandler *APIHandler) handleGetAtlasNodeInfo(request *restful.Request, response *restful.Response) {
	k8sClient, err := apiHandler.cManager.Client(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	...
	result := atlasnode.GetNodeInfo(k8sClient, name)

	log.Println("atlas node info debuginfo:===============>\n" + debugInfo(result))
...
func (apiHandler *APIHandler) handleAtlasDisplayFakeList(request *restful.Request, response *restful.Response) {
	k8sClient, err := apiHandler.cManager.Client(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
...
func (apiHandler *APIHandler) handleAtlasDisplayNodeList(request *restful.Request, response *restful.Response) {
	...
	dataSelect := parseDataSelectPathParameter(request)
	dataSelect.MetricQuery = dataselect.StandardMetrics
	nodeList, err := node.GetNodeList(k8sClient, dataSelect, apiHandler.iManager.Metric().Client())
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}

	atlasnodelist := atlasnode.AtlasNodeInfoList{*nodeList}

	log.Println("atlas node list debuginfo:===============>\n" + debugInfo(atlasnodelist))
...
func (apiHandler *APIHandler) handleAtlasDisplayJobList(request *restful.Request, response *restful.Response) {
	k8sClientConfig, err := apiHandler.cManager.Config(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}
  ...
  mpiJobClient, err := atlasjob.NewAtlasMpiJobClient(k8sClientConfig,namespace)

	dataSelect := parseDataSelectPathParameter(request)
	dataSelect.MetricQuery = dataselect.NoMetrics

	result := atlasjob.GetAtlasMpiJobList(mpiJobClient, namespace)
	log.Println("mpijob info list : ", debugInfo(result))
	response.WriteHeaderAndEntity(http.StatusOK, result)
}

func (apiHandler *APIHandler) handleAtlasDisplayJobInfo(request *restful.Request, response *restful.Response) {
	k8sClientConfig, err := apiHandler.cManager.Config(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
  ...
  mpiJobClient, err := atlasjob.NewAtlasMpiJobClient(k8sClientConfig, namespace)
	...
	result, err := atlasjob.GetAtlasMpiJob(mpiJobClient, k8sClient, namespace, name)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}
	log.Println("mpijob info : ", debugInfo(result))
	response.WriteHeaderAndEntity(http.StatusOK, result)
}
...
func (apiHandler *APIHandler) handleDeployAtlasExample(request *restful.Request, response *restful.Response) {
	k8sClient, err := apiHandler.cManager.Client(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	...
	atlasinteractiveSpec := new(atlasinteractive.DeploySpec)
	if err := request.ReadEntity(atlasinteractiveSpec); err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}
	if err := atlasinteractive.DeployApp(atlasinteractiveSpec, k8sClient); err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}
	log.Println(debugInfo(atlasinteractiveSpec))
	response.WriteHeaderAndEntity(http.StatusCreated, atlasinteractiveSpec)
}

func (apiHandler *APIHandler) handleAtlasTasksMpiJob(request *restful.Request, response *restful.Response) {
	cfg, err := apiHandler.cManager.Config(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	...
	atlasMpiJobSpec := new(atlasmpijob.MPIJobArgs)
	if err := request.ReadEntity(atlasMpiJobSpec); err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}
  ...
  isDeployed, err := atlasmpijob.DeployAtlasMpiJob(cfg, atlasMpiJobSpec, namespace)
	if !isDeployed {
		kdErrors.HandleInternalError(response, err)
		return
	}
	...
	log.Println("mpi job debuginfo:===============>\n" + debugInfo(atlasMpiJobSpec))

	response.WriteHeaderAndEntity(http.StatusCreated, atlasmpijob.MpiJobDeployResponse{
		Name:  atlasMpiJobSpec.Name,
		Error: errorMessage,
	})
}
...
func (apiHandler *APIHandler) handleAtlasDisplayDataList(request *restful.Request, response *restful.Response) {
	k8sClient, err := apiHandler.cManager.Client(request)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	...
	namespace := parseNamespacePathParameter(request)
	dataSelect := parseDataSelectPathParameter(request)
	pvclist, err := persistentvolumeclaim.GetPersistentVolumeClaimList(k8sClient, namespace, dataSelect)
	if err != nil {
		kdErrors.HandleInternalError(response, err)
		return
	}

	atlasdatalist := atlasdata.AtlasDataList(*pvclist)

	log.Println("atlas data debuginfo:===============>\n" + debugInfo(atlasdatalist))

	response.WriteHeaderAndEntity(http.StatusOK, atlasdatalist)
}
